<template>
  <div
    ...
    :key="consultation.id"
  >
    <div v-if="parseInt(consultation.state) == 1" class="mb-3 max-w-xs text-white rounded-xl pb-2 | bg-white bg-opacity-10 backdrop-filter backdrop-blur-xl shadow-xl">
      <div class="flex flex-wrap pt-1">
        <span
          class="ml-2 text-xs bg-blue-300 px-1 text-black rounded-md shadow-lg select-none"
          >{{ consultation.teacher_name }}</span
        >
        <span
